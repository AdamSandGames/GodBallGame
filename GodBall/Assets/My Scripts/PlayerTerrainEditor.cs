using UnityEngine;
// Heavily based on SC_TerrainEditor.cs
public class PlayerTerrainEditor : MonoBehaviour
{
    // Begin borrowed variable declarations
    public enum DeformMode { RaiseLower, Flatten, Smooth }
    DeformMode deformMode = DeformMode.RaiseLower;
    // string[] deformModeNames = new string[] { "Raise Lower", "Flatten", "Smooth" }; // GUI excluded
    ...
    TerrainData tData;
    // End borrowed variable declarations
    ...
    public void changeDeformMode(DeformMode newMode)
    {
        deformMode = newMode;
    }
    public DeformMode getDeformMode()
    {
        return deformMode;
    }
    public void loadTerrainChange()
    {
        Debug.Log("Editor Load Terrain");
        terrain = gameManager.getActiveTerrain();

        // code below moved from start method
    ...
    void Start() // Fix spot light, Debug print position, angles, target/direction,
    {
        ...
        // Change terrain layer to UI <Change to Handle in GameManager>
        // strength = 10;
        // area = 2;
        brushScaling();

        // added lines vvvvvvvv
        
    }
    // moved spotlight code
    void createSpotlight()
    {
        GameObject spotLightObj = new GameObject("SpotLight");
        spotLightObj.transform.SetParent(buildTarget);
        spotLightObj.transform.localPosition = new Vector3(0, 100, 0);
    ...
    void FixedUpdate()
    {
        // the borrowed fixedUpdate loop was moved into this if statement to control the playmode
        if(gameManager.getEditState() && gameManager.getGameState() == GameState.Playing)
        {
            raycastHit();
            // wheelValuesControl(); // Code excluded
    ...
    void raycastHit()
    {
        ...
        if (Physics.Raycast(ray, out hit, 3000, 1 << 5))
        {
            onTerrain = true;
            if (buildTarget)
            {
        ...
        else
        {
            if (buildTarget)
            {
                Vector3 curScreenPoint = new Vector3(Input.mousePosition.x, Input.mousePosition.y, 1000);
    ...
    void OnApplicationQuit() //Reset terrain height when exiting play mode
    {
        resetTerrain();
    }
